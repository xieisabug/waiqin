...
public class Recorder implements OnCompletionListener, OnErrorListener
{
	...
	public Recorder(Context context,String order_num)
	{
		mContext = context;
		File sampleDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + SAMPLE_DEFAULT_DIR + "/" + order_num);
		if (!sampleDir.exists())
		{
			sampleDir.mkdirs();
		}
	...
	public int getMaxAmplitude() //获取最大振幅
	{
	...
	public void reset()
	{
		...
		File sampleDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + SAMPLE_DEFAULT_DIR);
		if (!sampleDir.exists())
		{
			sampleDir.mkdirs();
		}
	...
	public void startRecording(int outputfileformat, String name, String extension, boolean highQuality, long maxFileSize)
	{
		stop();
		...
		RecorderService.startRecording(mContext, outputfileformat, mSampleFile.getAbsolutePath(), highQuality, maxFileSize);
