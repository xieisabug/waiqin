...
{
	// this is the log tag
	public static final String TAG = "com.sealion.servicemanage.mqtt.PushService";

	// the IP address, where your MQTT broker is running.
	private String MQTT_HOST =  getString(R.string.mqtt_host);
	...
	// MQTT broker.
	public static String MQTT_CLIENT_ID = "orderinfo";
    ...
    public class IMqttClientImpl extends com.sealion.serviceassistant.mqtt.IMqttClient.Stub
    {
		
		@Override
		public void sendMessageByMqtt(byte[] message) throws RemoteException
		{
			if (mConnection != null)
			{
				try
	...
	public void onStart(Intent intent, int startId)
	{
		...
		deviceID = sp.getInt("DB_ID", 0);
		
		log("Service started with intent=" + intent+ ", deviceID is "+deviceID);
	...
	private synchronized void connect()
	{
		...
		else
		{
			try
			{
				log("ip address 2 is : "+MQTT_HOST+" devicedID:"+deviceID);
	...
	public class MQTTConnection implements MqttSimpleCallback
	{
		 ...
		 */
		private int publishToTopic(String topicName, byte[] message) throws MqttException
		{
			int result = -1; // 未发送成功
			if ((mqttClient == null) || (mqttClient.isConnected() == false))
			{
			...
			{				
				result = mqttClient.publish(topicName, message, MQTT_QUALITY_OF_SERVICE, MQTT_RETAINED_PUBLISH);
				log("the send result is :" + result);
			}
			return result;
		}
		...
		public void publishArrived(String topicName, byte[] payload, int qos, boolean retained)
		{
			...
			//showNotification(s);
			log("topicName is :" + topicName);
			log("qos is :" + qos);
			log("retained is" +retained);
			log("Got message: " + s);
			//MqttAdhere.insertOrderMessageToDB(PushService.this, s);
		}
		...
		public void sendKeepAlive() throws MqttException
		{
			...
			publishToTopic(MQTT_CLIENT_ID + "/keepalive", mPrefs.getString(PREF_DEVICE_ID, "").getBytes());
		}
		
		public void sendMessage(byte[] str) throws MqttException
		{
			log("Sending latitute to server");
			publishToTopic(MQTT_CLIENT_ID+"/"+deviceID, str);
		}
	}
}