...
public class MQTTService extends Service implements MqttSimpleCallback {
    ...
    public static final String MQTT_MSG_RECEIVED_INTENT = "com.sealion.serviceassistant.mqtt.MSGRECVD";
    public static final String MQTT_MSG_RECEIVED_TOPIC = "com.sealion.serviceassistant.mqtt.MSGRECVD_TOPIC";
    public static final String MQTT_MSG_RECEIVED_MSG = "com.sealion.serviceassistant.mqtt.MSGRECVD_MSGBODY";

    // constants used to tell the Activity UI the connection status
    public static final String MQTT_STATUS_INTENT = "com.sealion.serviceassistant.mqtt.STATUS";
    public static final String MQTT_STATUS_MSG = "com.sealion.serviceassistant.mqtt.STATUS_MSG";

    // constant used internally to schedule the next ping event
    public static final String MQTT_PING_ACTION = "com.sealion.serviceassistant.mqtt.PING";
...
//    private short keepAliveSeconds = 20 * 60;
    ...
    public void onCreate() {
        ...
        Log.d(TAG, "onCreate brokerHostName : " + brokerHostName);
        topicName = settings.getString("topic", "");
        Log.d(TAG, "onCreate topicName : " + topicName);
        mqttClientId = settings.getString("USERNAME", "");
        Log.d(TAG, "onCreate mqttClientId : " + mqttClientId);
    ...
    synchronized void handleStart(Intent intent, int startId) {
        ...
        if (!isAlreadyConnected()) {
			...
			Notification notification = new Notification(R.drawable.logo_report,
														 "MQTT",
														 System.currentTimeMillis());
			notification.flags |= Notification.FLAG_ONGOING_EVENT;
			notification.flags |= Notification.FLAG_NO_CLEAR;
			Intent notificationIntent = new Intent(this, DesktopActivity.class);
    ...
    private void broadcastServiceStatus(String statusDescription) {
        ...
        Log.d(TAG, "broadcastServiceStatus statusDescription:" + statusDescription);
    ...
    private void broadcastReceivedMessage(String topic, String message) {
        ...
        //   in the app GUI
        Log.d(TAG, "broadcastReceivedMessage topic:" + topic + " , message:" + message);
    ...
    private void notifyUser(String alert, String title, String body) {
        NotificationManager nm = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
        Notification notification = new Notification(R.drawable.logo_report, alert,
                System.currentTimeMillis());
    ...
    public void connectionLost() throws Exception {
        ...
        if (!isOnline()) {
            ...
            Log.d(TAG, "Connection lost - no network connection");
            //
    ...
    public void publishArrived(String topic, byte[] payloadbytes, int qos, boolean retained) {
        ...
        Log.d(TAG, "publishArrived qos:" + qos);
        Log.d(TAG, "publishArrived topic:" + topic);
        Log.d(TAG, "publishArrived messageBody:" + messageBody);

        MqttAdhere mqttAdhere = new MqttAdhere();

        if (topic.contains("new")) //新工单
        {
            mqttAdhere.insertOrderMessageToDB(this, messageBody);
        } else if (topic.contains("notification")) //通知
        {
            mqttAdhere.insertNoticeMessageToDB(this, messageBody, this.getResources().getString(R.string.request_url), sp.getString("AREAID", ""));
        } else if (topic.contains("cancel_order")) //改派结果
        {
            mqttAdhere.DisposeOrderChange(this, messageBody);
        } else if (topic.contains("data_update_notification")) //
        {
		...
		if (addReceivedMessageToStore(topic, messageBody))
		{
			...
			Log.d(TAG, "New data received" + messageBody);
		}
		*/
    ...
    private void defineConnectionToBroker(String brokerHostName) {
        ...
        Log.d(TAG, "defineConnectionToBroker mqttConnSpec: " + mqttConnSpec);
        try {
            // define the connection to the broker
    ...
    private boolean connectToBroker() {
        try {
            Log.d(TAG, "connectToBroker mqttClientId:" + mqttClientId);
            Log.d(TAG, "connectToBroker cleanStart:" + cleanStart);
            Log.d(TAG, "connectToBroker keepAliveSeconds:" + keepAliveSeconds);
            // try to connect
            ...
            scheduleNextPing();
            Log.d(TAG, "connectToBroker Connected.................");
            return true;
        } catch (MqttException e) {
            // something went wrong!
            e.printStackTrace();
            ...
            Log.d(TAG, "Unable to connect - will retry later");
    ...
    private void subscribeToTopic(String topicName) {
        ...
        Log.d(TAG, "subscribeToTopic isAlreadyConnected():" + isAlreadyConnected());
        if (!isAlreadyConnected()) {
            ...
            try {
                ...
                Log.e(TAG, "subscribeToTopic:" + Arrays.toString(topics));
